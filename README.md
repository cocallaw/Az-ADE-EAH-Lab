...
> **Key Vault purge protection:** The templates enable purge protection with a 7-day soft-delete retention period. After resource group deletion, the Key Vault name remains **reserved for 7 days** and cannot be reused. For repeated lab runs, use a different resource group or Key Vault prefix. To purge a soft-deleted vault immediately: `az keyvault purge --name <KV-NAME>`. See [Key Vault soft-delete overview](https://learn.microsoft.com/en-us/azure/key-vault/general/soft-delete-overview) for details.
